...
async def get_all_distribution(request: Request, session: db_session, pagination_params: pagination_params_dep):
    result = await DistributionServiceAsync(db_session=session, request=request).select_all_distributions(pagination_params)
    return {'message': result}

@router.get('/relation-ship', description='Вывод всех продаж с отношениями между таблицами')
async def get_all_distribution_rel(request: Request, session: db_session, pagination_params: pagination_params_dep):
    result = await DistributionServiceAsync(db_session=session, request=request).select_all_distributions_rel(pagination_params)
    return {'message': result}
...
@router.get('/{distribution_id}/relation-ship', description='Вывод продаж по id с отношениями между таблицами')
async def get_distribution_by_id_rel(distribution_id: int, request: Request, session: db_session):
    result = await DistributionServiceAsync(db_session=session, request=request).select_distribution_by_id_rel(distribution_id)
    return {'message': result}
...
async def delete_distribution_by_id(distribution_id, request: Request, session: db_session):
