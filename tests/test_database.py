...
from src.config import settings, Base, db_engine_helper
...
def setup_db():
    Base.metadata.drop_all(db_engine_helper._engine_sync)
    Base.metadata.create_all(db_engine_helper._engine_sync)
    yield
    Base.metadata.drop_all(db_engine_helper._engine_sync)

@pytest.fixture(scope="module", autouse=True)
def default_records():
    with db_engine_helper._engine_sync.connect() as connection:
        ...
        users = text("INSERT INTO users(fio, phone_number, address, email, password, workload, salary, role, post, bonus) "
                     "VALUES ('user1', '+790213213', 'address', 'user1@gmail.com', '123', 'PARTTIME', '10000', 'USER', 'MANAGER', '0'), "
                            "('user2', '+790213213', 'address', 'user2@gmail.com', 'qwerty', 'PARTTIME', '10000', 'USER', 'MANAGER', '0')")
        readers = text("INSERT INTO readers(fio, phone_number, email, password, gender, discount) "
                     "VALUES ('readers1', '+790213234', 'readers1@gmail.com', 'sadfasd', 'm', '0'), "
                            "('readers2', '+790213234', 'readers2@gmail.com', 'fdg345', 'w', '0')")
...
@pytest.mark.parametrize("fio, phone_number, email, password, gender, discount, exception",
                         [('fio1', '+7902324', 'test', '123', 'm', '0', nullcontext()),
                          ('fio2', '+7902324', 'test', '123', 'm', '0', nullcontext()),
                          ('fio4', '+7902324', 'test', '123', 'm', '0', nullcontext()),
                          ('fio6', '+7902324', 'test', '123', 'm', '0', nullcontext())])
def test_create_records_for_readers(fio, phone_number, email, password, gender, discount, exception):
    with exception:
        with db_engine_helper._engine_sync.connect() as connection:
            stmt = text("INSERT INTO readers(fio, phone_number, email, password, gender, discount) "
                        "VALUES (:fio, :phone_number, :email, :password, :gender, :discount)")
            connection.execute(stmt, {'fio': fio, 'phone_number': phone_number, 'email': email,
                                      'password': password, 'gender': gender, 'discount': discount})

@pytest.mark.parametrize("fio, phone_number, address, email, password, workload, salary, role, post, bonus, exception",
                         [('fio1', '+7902324', 'address', 'test', '123', 'PARTTIME', '0', 'USER', 'MANAGER', '0', nullcontext()),
                          ('fio2', '+7902324', 'address', 'test', '123', 'PARTTIME', '0', 'USER', 'MANAGER', '0', nullcontext()),
                          ('fio4', '+7902324', 'address', 'test', '123', 'PARTTIME', 'FDGSDFG', 'USER', 'MANAGER', '0', pytest.raises(Exception)),
                          ('fio5', '+7902324', 'address', 'test', '123', 'PARTTIME', '0', 'USER', 'MANAGER', '0', nullcontext())])
def test_create_records_for_users(fio, phone_number, address, email, password, workload, salary, role, post, bonus, exception):
    with exception:
        with db_engine_helper._engine_sync.connect() as connection:
            stmt = text("INSERT INTO users(fio, phone_number, address, email, password, workload, salary, role, post, bonus) "
                        "VALUES (:fio, :phone_number, :address, :email, :password, :workload, :salary, :role, :post, :bonus)")
            connection.execute(stmt, {'fio': fio, 'phone_number': phone_number, 'address': address,'email': email,
                                      'password': password, 'workload': workload, 'salary': salary,
